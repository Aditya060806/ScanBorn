## ScanBorn System Design Blueprint v2

Snapdragon Multiverse Hackathon, Noida | System Design Blueprint v2 (Reviewed)

July 2026

- [0. Reviewer’s Note: What Changed From v1 and Why](file:///home/claude/scanborn/blueprint.html#reviewers-note-what-changed-from-v1-and-why)

- [1. Vision & Product Positioning](file:///home/claude/scanborn/blueprint.html#vision-product-positioning)

- [2. Problem Statement](file:///home/claude/scanborn/blueprint.html#problem-statement)

- [3. Pipeline Landscape: Qualcomm/Edge vs. Cloud‑Centric Robotics Stacks](file:///home/claude/scanborn/blueprint.html#pipeline-landscape-qualcommedge-vs.-cloudcentric-robotics-stacks)

- [4. Qualcomm Ecosystem Alignment](file:///home/claude/scanborn/blueprint.html#qualcomm-ecosystem-alignment)

- [5. OnePlus, CodeMate, and Sarvam Integration](file:///home/claude/scanborn/blueprint.html#oneplus-codemate-and-sarvam-integration)

- [6. End‑to‑End System Architecture](file:///home/claude/scanborn/blueprint.html#endtoend-system-architecture)

- [7. Detailed Sequence Diagram](file:///home/claude/scanborn/blueprint.html#detailed-sequence-diagram)

- [8. Component‑by‑Component Design](file:///home/claude/scanborn/blueprint.html#componentbycomponent-design)

- [9. Data Flow Diagram](file:///home/claude/scanborn/blueprint.html#data-flow-diagram)

- [10. World Coordinate System & Localization](file:///home/claude/scanborn/blueprint.html#world-coordinate-system-localization)

- [11. Sim‑to‑Real Pipeline](file:///home/claude/scanborn/blueprint.html#simtoreal-pipeline)

- [12. RL / Policy‑Learning Pipeline](file:///home/claude/scanborn/blueprint.html#rl-policylearning-pipeline)

- [13. Robot Deployment](file:///home/claude/scanborn/blueprint.html#robot-deployment)

- [14. Edge AI Optimization (Qualcomm AI Hub / QAIRT)](file:///home/claude/scanborn/blueprint.html#edge-ai-optimization-qualcomm-ai-hub-qairt)

- [15. Repository Structure](file:///home/claude/scanborn/blueprint.html#repository-structure)

- [16. API Specification (Orchestrator REST Surface)](file:///home/claude/scanborn/blueprint.html#api-specification-orchestrator-rest-surface)

- [17. Folder Structure, UML, and DB Schema](file:///home/claude/scanborn/blueprint.html#folder-structure-uml-and-db-schema)

- [17.1 Core Class Diagram (text form)](file:///home/claude/scanborn/blueprint.html#core-class-diagram-text-form)

- [17.2 DB Schema (Postgres/SQLite — shared metadata store)](file:///home/claude/scanborn/blueprint.html#db-schema-postgressqlite-shared-metadata-store)

- [18. ROS2 and Unity Project Structure](file:///home/claude/scanborn/blueprint.html#ros2-and-unity-project-structure)

- [19. SDK Design](file:///home/claude/scanborn/blueprint.html#sdk-design)

- [20. Plugin Architecture](file:///home/claude/scanborn/blueprint.html#plugin-architecture)

- [21. Deployment Architecture](file:///home/claude/scanborn/blueprint.html#deployment-architecture)

- [22. Docker & Kubernetes (Optional)](file:///home/claude/scanborn/blueprint.html#docker-kubernetes-optional)

- [23. Security Considerations](file:///home/claude/scanborn/blueprint.html#security-considerations)

- [24. Performance Targets](file:///home/claude/scanborn/blueprint.html#performance-targets)

- [25. Scalability](file:///home/claude/scanborn/blueprint.html#scalability)

- [26. Open‑Source Components Used & Licensing](file:///home/claude/scanborn/blueprint.html#opensource-components-used-licensing)
...
| manipulation task is the stretch goal). ScanBorn.train() |
...
ScanBorn is an edge‑first Physical AI operating layer: point a phone at any real space, and within minutes get a simulation‑ready digital twin, a robot that can be taught a task by voice in any language, and a policy that runs — fully offline — on Snapdragon silicon.

The positioning bet is deliberate and matches where Qualcomm is pushing the industry: NVIDIA’s sim‑to‑real story (Isaac Sim, Omniverse, Jetson) is built around a cloud/workstation GPU compute model — heavy reconstruction and training happen on datacenter‑class hardware, and the edge device is mostly a thin inference client. ScanBorn instead runs the entire pipeline — capture, semantic understanding, twin generation, adaptation, and deployment — on a phone, an AI PC, and a microcontroller‑class board, with the cloud used only optionally, for the heaviest one‑time reconstruction pass. That is the wedge: “digital twins and robot learning that don’t need a data-center,” aimed squarely at Qualcomm’s push into robotics/physical AI and gaming/simulation as an edge-compute story, not a cloud story.

ScanBorn is not pitched as “a robot demo.” It is pitched as a developer platform — an SDK and a set of adapters — so that after the hackathon, anyone can plug in a new robot, a new simulator, or a new capture device by implementing a three‑method interface, without touching the pipeline.
...
ScanBorn collapses “new environment → working, safe, task‑capable robot” from days/weeks to minutes, by automating capture → reconstruction → semantic labeling → simulation‑ready twin → policy adaptation → edge‑optimized deployment as one pipeline, callable as one SDK, running on commodity edge hardware rather than a cloud GPU cluster.


## 3. Pipeline Landscape: Qualcomm/Edge vs. Cloud‑Centric Robotics Stacks

This is a fair comparison, not a takedown — the incumbent cloud pipelines are mature and powerful for their target use case (large fleets, heavy photorealistic sim, massive parallel RL). ScanBorn targets a different, currently underserved point on the map: single‑environment, same‑day, offline‑capable onboarding.

| Dimension | Cloud/Workstation‑GPU stacks (e.g. Isaac | ScanBorn (Qualcomm edge‑first) |
| --- | --- | --- |
...
OnePlus. The OnePlus 15 is not a generic “any Android phone” — it’s the actual capture instrument: Snapdragon 8 Elite NPU pre‑processing for depth/feature extraction, and its camera stack is the sole source of the raw RGB‑D+IMU stream. The capture/ module’s public interface (ScanBornCapture SDK) is written against this device’s camera2/ARCore session so OnePlus’s hardware is structurally the entry point of the whole platform, not a swappable commodity — the demo narrative literally starts with “pick up the OnePlus 15.”
...
- 6. Transfer: identical ROS2 node graph runs in sim and on the real UNO Q‑driven rover; only the ScanBornRobot adapter implementation changes (SimRobot vs. UnoQRobot), so “sim‑to‑real” is literally a one‑line adapter swap, not a re‑implementation.
...
- ScanBorn.train() is intentionally scoped as fine‑tune, not train‑from‑scratch: this is both the technically honest scope for 24 hours and — per the AI Hub guide — the only thing worth quantizing and deploying in that time.

- Evaluation: ScanBorn.evaluate() runs N sim episodes post‑fine‑tune and reports a success‑rate delta (before/after), which is the number shown on the dashboard and in the presentation.
...
ScanBornRobot (abstract) TaskPlanner (abstract)
...
├─ SimRobot : ScanBornRobot
└─ UnoQRobot : ScanBornRobot
ScanBornSimulator (abstract)
├─ load_twin(twin)
├─ spawn_robot(robot)
└─ step()
└─ UnityROS2Sim : ScanBornSimulator
```
...
```
src/
├── scanborn_bringup/ # launch files, per-robot params
├── scanborn_msgs/ # TaskGraph.msg, Waypoint.msg custom types
├── scanborn_adapters/ # SimRobot / UnoQRobot nodes implementing ScanBornRobot
├── scanborn_nav/ # Nav2 config (costmap, planner, controller params)
└── scanborn_bridge/ # thin node talking to the UNO Q Bridge over the app_bridge helper
```
...
Twin generation runs headless via Unity -batchmode -executeMethod ScanBorn.GenerateTwin.Run -sceneId {twin_id}, callable directly from the twin/ service — no manual Editor interaction needed at demo time.
...
New robot → implement ScanBornRobot, register in robot/adapters/registry.py. New simulator (e.g. swap Unity for a lighter engine) → implement ScanBornSimulator. New reasoning provider → implement TaskPlanner. The orchestrator only ever talks to the abstract interface, never a concrete class.
