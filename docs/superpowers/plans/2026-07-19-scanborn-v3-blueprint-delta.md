# ScanBorn v3 Blueprint Delta Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Bring the already-complete v2 pipeline (81/81 tests passing, every stage real per `docs/superpowers/plans/2026-07-18-scanborn-v2-implementation.md`) in line with `ScanBorn_Implementation_Blueprint_v3.pdf`: Scaniverse-fed splat capture, ArUco coordinate alignment, on-policy RL training via Unity ML-Agents, and a 6-step guided web wizard that replaces the SDK as the product's front door.
...
| `sdk/scanborn/client.py` | Real, 8 verbs, 82% coverage |
...
Files: `README.md`, `sdk/scanborn/client.py` docstring only.
