...
private fun BubbleCanvas(state: BubbleState, dragging: Boolean) {
    ...
    // ── ScanBorn path phase (used for THINKING / GENERATING trace) ────────────
    ...
    Canvas(
        ...
        when (state) {
            BubbleState.IDLE, BubbleState.DONE ->
                drawScanBornStatic(cx, cy, r * 0.40f)

            BubbleState.THINKING ->
                drawScanBornTrace(cx, cy, r * 0.40f, tracePhase, strokeAlpha = 0.75f)

            BubbleState.SCANNING ->
                drawScanBornStatic(cx, cy, r * 0.40f, alpha = 0.55f)

            BubbleState.GENERATING ->
                drawScanBornTrace(cx, cy, r * 0.40f, tracePhase, strokeAlpha = 0.85f)
        }
    }
}

// ── Static scanborn glyph ─────────────────────────────────────────────────────

private fun DrawScope.drawScanBornStatic(
    cx: Float, cy: Float, halfWidth: Float,
    alpha: Float = 0.82f
) {
    val path = scanbornPath(cx, cy, halfWidth)
    drawPath(
        path  = path,
...
// ── Animated trace scanborn ───────────────────────────────────────────────────

private fun DrawScope.drawScanBornTrace(
    cx: Float, cy: Float, halfWidth: Float,
...
// ── ScanBorn lemniscate path builder ─────────────────────────────────────────
...
private fun scanbornPath(cx: Float, cy: Float, hw: Float): Path {
